  ...
  var phoneno = /^\d{10}$/;
   var alphaValidate = /^[a-zA-Z]+$/;   
    if(status.value.length == 0) {
        ...
        document.getElementById("status").innerHTML = "*Please enter  status*";
        fname.focus();
...
.round {
 ...
 <script>
  ...
  <script>
function myFunction() {
  alert("Successfully inserted clockin status!");
}
</script>
...
<div class="header">
  <div class="header-right">
    ...
    <a href="#">Log out</a>
        ...
        <div class="h1">Permanent Employee Attendance
<a href="addAttendance.jsp" class="next round">&#8250;</a></div>
		...
		<table class="table table-condensed"  style="  background-color: #fff; padding: 30px; width: 560px;">
		 ...
		 <%
			try{
				...
				String sql ="select * from per_employee";
			...
			%>
		  <tr>
		  <form action="PerEmpAttenPresentServlet" method="post">
		  
		
		  <td><input class="logText"   type = "hidden" name="date" id="date" value="<fmt:formatDate value="${date}" type="date" pattern="YYYY-MM-dd"/> " > </input></td>
		    <td><input class="noOutLine" name = "employeeId" style="border:none;width :40px" value="<%=resultSet.getString("employeeId") %>"></input></td>
		    <td><input class="noOutLine"  name = "firstName" style="border:none;width :60px"value="<%=resultSet.getString("firstName") %>"></input></td> 
		    <td> <input class="noOutLine" name = "lastName" style="border:none;width :80px"value="<%=resultSet.getString("lastName") %>"></input></td>
		    <td><input class="noOutLine"  name = "designation" style="border:none;width :150px"value="<%=resultSet.getString("designation") %>"></td>
		    <td><input class="noOutLine" name = "section" style="border:none;width :150px"value="<%=resultSet.getString("section") %>"></td>
		      <td> <input  type="radio" name="status" value="Present" id="present" onchange="validateNewEmp()" required >
			                        <lable style="color:black;"> Present </lable>
			                        
			                        <input class="" type="radio" name="status" value="Absent" id="absent" onchange="newEmployeeValidate" >
			                        <label style="color:black;">  Absent</label> </td>
		    <td> <input name="clockin" type="time" id="time" value= "00:00" style = "width :110px" ></td>
		       <td><button name="create" id = "submitbtn" type="submit"  class="btn btn-success" onclick="myFunction()"> OK</button></td>
		          <td><input name="basicSalary" type="hidden" value="<%=resultSet.getString("basicSalary") %>"> </td>
