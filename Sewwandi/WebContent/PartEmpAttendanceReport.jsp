...
<h3>HOTEL SEWWANDI</h3></center>
  ...
  <h4 style = "color:red"> Search results from  <% out.println(date1);%> to <% out.println(date2);%></h4>
	  ...
	  <h5>Attendance Summary report<h5><form>
<input type="button" value="Print report" onClick="window.print()" style=" color: black ;background:#fff;border: none; text-decoration: underline;padding-left: 10px; padding-right: 650px; 
	padding-top: 30px; 
	padding-bottom: 10px; 
outline:none;"><a style=" color: black ; text-align: right;" href="PerEmpAttendanceDetails.jsp">Search again</a>
		...
		<table class="center"  style="background-color: #fff; width: 900px;">
		 ...
		 <%
			try{
				...
				String sql = "select *,count(case when status ='Absent'  then 1 end) as absent_count ,count(case when status ='Present' then 1 end) as present_count from sewwandi.part_empattendance where date between '"+date1+"' and '"+date2+"' group by employeeId order by present_count desc;";
		  ...
		  <form action="editPerEmpattendance.jsp" method="post">
		    ...
		    <td><%=resultSet.getString("section") %> <input name="section" type="hidden" value="<%=resultSet.getString("section") %>"></td>
		       ...
		       <td><%=resultSet.getString("present_count") %> <input name="fname" type="hidden" value="<%=resultSet.getString("Present_count") %>"> </td> 
		    <td><%=resultSet.getString("absent_count") %> <input name="fname" type="hidden" value="<%=resultSet.getString("absent_count") %>"> </td> 
